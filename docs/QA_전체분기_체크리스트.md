...
| C-04 | 사주 단독 화면 | F6 | `/lens/saju` | **일주 값 + 한 줄 해석 + 질문 하나** · DEMO 문구 0 · 음력은 계산하지 않고 이유 표시 (260914 UT 후속 P0 — Premium 렌즈와 같은 `readSajuDay`) | 혼합(P0-01) |
...
| C 렌즈 | 11 | 11 | C-04는 260914부터 실제 일주 표시 · C-09~11은 v1.46.3 |
...
- ~~**사주 단독 화면(`/lens/saju`)** — '엔진 미연결' 안내~~ → 260914 UT에서 'DEMO' 노출로
  실제 혼동이 확인돼 해결. 단독 화면도 Premium 렌즈와 같은 일주 엔진을 쓴다(FREE는 일주 + 한 줄 + 질문 하나).
