...
/**
 * POST /api/ai/premium-zodiac-lens  (v1.46 AI Lens · PHASE C)
 *
 * **이미 결정론 엔진이 계산한** 별자리 관계 렌즈 결과에 해석 문단만 붙인다.
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!result.ok) {
    logAi({ requestId, task: 'premium-zodiac-lens', status: 'fail', durationMs, reason: result.reason });
    return failureResponse(result.reason, requestId);
  }

  logAi({ requestId, task: 'premium-zodiac-lens', status: 'ok', durationMs });
