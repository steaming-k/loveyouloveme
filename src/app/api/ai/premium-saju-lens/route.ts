...
/**
 * POST /api/ai/premium-saju-lens  (v1.46 AI Lens · PHASE B)
 *
 * **이미 결정론 엔진이 계산한** 사주 관계 렌즈 결과에 해석 문단만 붙인다.
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!result.ok) {
    logAi({ requestId, task: 'premium-saju-lens', status: 'fail', durationMs, reason: result.reason });
    return failureResponse(result.reason, requestId);
  }

  logAi({ requestId, task: 'premium-saju-lens', status: 'ok', durationMs });
