...
const CONNECTION_REASON: Record<PremiumChapterKind, string | null> = {
  ...
  closeness_distance:
    '연락과 개인 시간은 따로 보면 다른 기준인데, 관계에서의 거리라는 점에서는 같이 볼 수 있어. 그래서 두 기준을 하나로 묶어봤어.',
  hidden_priority:
    '네가 중요하다고 말한 것과, 실제 관계에서 힘들었다고 적은 지점이 같은 방향인지 같이 봤어.',
  conflict_needs:
    '갈등은 한 가지 답만으로는 방식이 안 보여. 네가 답한 기준과 실제로 겪은 걸 같이 놓아야 어떤 방식을 원했는지가 드러나는 자리야.',
  affection_exchange:
    '표현은 얼마나 자주였는지보다 어떤 방식이었는지에서 갈려. 그걸 보려면 서로 다른 자리에서 답한 내용이 같이 있어야 해.',
  tune_with_target:
    '상대 정보만으로는 맞춰볼 지점이 나오지 않아. 네 기준과 같이 놓아야 어디에서 기대가 다른지가 보여.',
  past_and_now:
    '서로 다른 시점에 따로 저장된 기록이야. 두 기록에서 같은 기준이 보일 때만 같이 볼 수 있어서, 그때만 이 자리를 만들어.',
  uncertainty: null,
  next_check: null,
  closing: null,
  self_profile:
...
const CHECK_SELF: Record<PremiumChapterKind, string> = {
  ...
  tune_with_target: '점수를 높이려 하기보다 기대가 다른 기준 하나를 먼저 확인해봐.',
  uncertainty:
    '여긴 아직 빈칸이야. 억지로 채우지 말고, 어떤 답이 쌓이면 채워지는지만 기억해둬.',
  next_check: '여기 있는 건 결론이 아니야. 하나만 골라서 다음에 확인해봐.',
  past_and_now:
    '두 기록을 나란히 놓을 수 있을 때만 변화라고 부를게. 다음 기록에서 같은 기준을 한 번 더 확인해봐.',
...
const CHECK_FORMER: Partial<Record<PremiumChapterKind, string>> = {
  ...
  tune_with_target:
    '그때 기대가 달랐던 기준 하나를, 다음에 더 일찍 확인할 목록에 올려둬.',
